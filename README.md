...
`함수 정의 → Docstring → 코드(주석) → 풀이 메모` 순서로 작성한다. 풀이 메모는 별도
파일로 분리하지 않고 **해당 풀이 파일 맨 아래 주석**으로 둔다. 메모가 그 코드에만
해당하는 시행착오 기록이라 코드와 떨어뜨리면 문맥이 끊기기 때문이다.
...
막혔던 지점·오류 원인·손으로 돌려본 과정처럼 **그 문제를 풀며 겪은 것**은 파일 맨 아래
구분선 안에 날짜별로 남긴다. 코드를 고쳐도 이 기록은 지우지 않는다.
